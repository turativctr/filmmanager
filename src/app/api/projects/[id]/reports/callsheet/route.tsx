...
import { CallSheetDocument } from "@/lib/pdf/call-sheet-document";
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const buffer = await renderToBuffer(<CallSheetDocument data={data} />);

  return new NextResponse(new Uint8Array(buffer), {
    status: 200,
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="${gerarNomeArquivo({ projeto: project, tipo: "OD", variante: `Diaria${data.shootDay.numeroDia}`, ext: "pdf" })}"`,
    },
  });
}
