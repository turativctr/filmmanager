...
import { BreakdownDocument } from "@/lib/pdf/breakdown-document";
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const buffer = await renderToBuffer(<BreakdownDocument data={data} />);

  return new NextResponse(new Uint8Array(buffer), {
    status: 200,
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="${gerarNomeArquivo({ projeto: project, tipo: "AT", variante: `Diaria${data.shootDay.numeroDia}`, ext: "pdf" })}"`,
    },
  });
}
